...
class LoanApplication(BaseModel):
    ...
    loan_term: int = Field(
        ..., ge=1, le=20, description="Repayment duration (1–20 years)"
    )
    cibil_score: float = Field(
        ..., ge=300, le=900, description="Credit score (300–900)"
    )
    residential_assets_value: float = Field(
        0, ge=0, description="Value of residential properties"
    )
    commercial_assets_value: float = Field(
        0, ge=0, description="Value of commercial properties"
    )
